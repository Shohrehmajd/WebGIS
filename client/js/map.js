...
var wmsLayerGroup = wmsLayers.map(function(layerInfo) {
    return new ol.layer.Tile({
        ...
        visible: true,  // Sichtbar für die Legende
...
var map = new ol.Map({
    target: 'map',
    layers: [rasterLayer].concat(wmsLayerGroup), // Nur einmal hinzufügen
    view: new ol.View({
    ...
    if (!document.getElementById(`${layerInfo.layerName}_checkbox`)) {
        ...
        toggle.addEventListener('click', function() {
            if (legendContent.style.display === 'none') {
                ...
                toggle.innerHTML = '&#x25BC;';  // Pfeil nach unten
            } else {
                legendContent.style.display = 'none';
                toggle.innerHTML = '&#x25BA;';  // Pfeil nach rechts
            }
        });
    }
}
...
function enablePanMode() {
    ...
    map.getViewport().style.cursor = 'grab'; // Hand-Icon als Mauszeiger
}
...
function removeCurrentInteraction() {
    ...
    map.getViewport().style.cursor = ''; // Cursor auf Standard zurücksetzen
}

// Zoom-Funktion
document.getElementById('zoom-in').addEventListener('click', function() {
    var view = map.getView();
    view.setZoom(view.getZoom() + 1); // Zoom-In
});

document.getElementById('zoom-out').addEventListener('click', function() {
    var view = map.getView();
    view.setZoom(view.getZoom() - 1); // Zoom-Out
});
...
function addMeasureInteraction(type) {
    ...
    measureInteraction = new ol.interaction.Draw({
        source: new ol.source.Vector(), // Temporärer Layer, in dem die Zeichnung gespeichert wird
    ...
    measureInteraction.on('drawstart', function (evt) {
        ...
        geom.on('change', function () {
            ...
            if (geom.getType() === 'Polygon') {
                output = formatArea(geom); // Berechnung der Fläche
            } else if (geom.getType() === 'LineString') {
                output = formatLength(geom); // Berechnung der Entfernung
            }
    ...
    measureInteraction.on('drawend', function () {
        ...
        measureInteraction = null; // Beende die Messinteraktion
        map.getViewport().style.cursor = ''; // Setze den Cursor auf Standard zurück
    });
}
...
function formatLength(line) {
    ...
    let output;
    if (length > 100) {
        output = (length / 1000).toFixed(2) + ' km'; // Kilometereinheit
    } else {
        output = length.toFixed(2) + ' m'; // Metereinheit
    }
    return output;
}
...
function formatArea(polygon) {
    ...
    let output;
    if (area > 10000) {
        output = (area / 1000000).toFixed(2) + ' km²'; // Quadratkilometer
    } else {
        output = area.toFixed(2) + ' m²'; // Quadratmeter
    }
    return output;
}
...
document.getElementById('measure-button').addEventListener('click', function () {
    ...
    if (type === 'line' || type === 'area') {
        ...
        map.getViewport().style.cursor = 'crosshair'; // Cursor zum Fadenkreuz ändern
    } else {
...
document.querySelector('[data-mode="pan"]').addEventListener('click', function() {
    enablePanMode(); // Pan-Modus aktivieren
});
...
function exportMap() {
    map.once('rendercomplete', function() {
        ...
        link.download = 'map-export.png'; // Dateiname
        link.click(); // Klick zum Auslösen des Downloads
    });
...
document.getElementById('exportButton').addEventListener('click', function() {
    exportMap(); // Export-Funktion aufrufen
});
function printMap() {
...
document.getElementById('printButton').addEventListener('click', function() {
    printMap();  // Druckfunktion aufrufen
});
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (infoButton && infoContainer && closeButton) {
        ...
        infoButton.addEventListener('click', function() {
            infoContainer.style.display = 'block'; // Info-Container anzeigen
        });

        // Event-Listener für den Close-Button
        closeButton.addEventListener('click', function() {
            infoContainer.style.display = 'none'; // Info-Container verstecken
        });
    } else {
...
var scaleLineControl = new ol.control.ScaleLine({
    units: 'metric',  // Verwende metrische Einheiten
    bar: true,        // Zeige eine Balkenanzeige
    steps: 4,         // Anzahl der Schritte in der Balkenanzeige
    text: true,       // Text anzeigen
    minWidth: 100     // Mindestbreite der Balkenanzeige
});
...
map.on('singleclick', function(evt) {
    ...
    popupContent.innerHTML = '';  // Popup-Inhalt leeren
    var featureFound = false;  // Variable zum Überprüfen, ob ein Feature gefunden wurde

    // WMS-Layer durchgehen, um Feature-Informationen abzurufen
    var promises = wmsLayerGroup.map(function(layer) {
        if (layer.getVisible()) {  // Nur sichtbare Layer berücksichtigen
            var source = layer.getSource();
            var url = source.getFeatureInfoUrl(
                coordinate, viewResolution, 'EPSG:25832',  // Nutze die korrekte Projektion
                { 'INFO_FORMAT': 'application/json' }  // Feature-Info als JSON
            );
                ...
                return fetch(url)
                    ...
                    .then(function(data) {
                        if (data.features && data.features.length > 0) {
                            featureFound = true;  // Feature wurde gefunden

                            var featureInfo = data.features[0];
                            var featureName = featureInfo.properties.name || 'Unbenannt';  // Fallback-Name
        ...
        return Promise.resolve();  // Rückgabe einer leeren Promise, wenn kein Layer sichtbar ist
    });

    // Popup anzeigen, wenn ein Feature gefunden wurde
    Promise.all(promises).then(function() {
        if (featureFound) {
            overlay.setPosition(coordinate);  // Popup an der Position anzeigen
        } else {
            overlay.setPosition(undefined);  // Popup verstecken, wenn nichts gefunden wurde
        }
    });
});
...
map.on('pointermove', function(evt) {
    var coordinate = evt.coordinate;
    var transformedCoord = ol.proj.transform(coordinate, 'EPSG:25832', 'EPSG:4326'); // In LonLat umwandeln
    var lon = transformedCoord[0].toFixed(6); // Längengrad
    var lat = transformedCoord[1].toFixed(6); // Breitengrad
...
// Funktion für die Nominatim-Suche (mit Popup)
    ...
    fetch(url)
        ...
        .then(data => {
            if (data && data.length > 0) {
                ...
                overlay.setPosition(coordinate); // Popup auf die Koordinate setzen
            } else {
...
document.getElementById('geometryTree').addEventListener('click', function() {
    ...
    treeMenu.classList.toggle('hidden'); // Sichtbarkeit umschalten
});
            ...
            fetch(url)
                ...
                .then(function(data) {
                    if (data.features && data.features.length > 0) {
                        ...
                        content.innerHTML = formattedInfo;  // Popup-Inhalt setzen
                    } else {
                        content.innerHTML = `<p>Keine Feature-Informationen gefunden.</p>`;
                    }
                    overlay.setPosition(coordinate);  // Popup an Position anzeigen
                })
                .catch(function(error) {
                    console.error('Fehler bei der GetFeatureInfo-Anfrage:', error);
                    content.innerHTML = `<p>Fehler bei der Abfrage.</p>`;
                    overlay.setPosition(coordinate);  // Popup anzeigen bei Fehler
                });
        } else {
            showCoordinatePopup(coordinate);  // Zeige die Koordinaten, wenn kein URL verfügbar ist
        }
    } else {
        showCoordinatePopup(coordinate);  // Zeige die Koordinaten, wenn kein WMS-Layer vorhanden ist
    }
});
...
function exportMap() {
    html2canvas(document.querySelector('#map'), {
        useCORS: false,  // Versuche es ohne CORS, um den Fehler zu umgehen
        ignoreElements: (element) => element.tagName === 'IMG'  // Bilder von der Aufnahme ausschließen
    }).then(function(canvas) {
        var link = document.createElement('a');
        link.href = canvas.toDataURL('image/png');
        link.download = 'map-export.png';  // Dateiname des Exportbildes
        link.click();  // Klickt auf den Download-Link, um das Bild herunterzuladen
    }).catch(function(error) {
...
var vectorSource = new ol.source.Vector({
    wrapX: false  // Verhindert das unendliche Wiederholen von Geometrien
});

// Vektor-Layer für das Zeichnen und Anzeigen von Geometrien
var vectorLayer = new ol.layer.Vector({
    source: vectorSource,  // Setze den vectorSource als Datenquelle
    style: new ol.style.Style({
        stroke: new ol.style.Stroke({
            color: '#ffcc33',  // Farbe für Linien
            width: 2
        }),
        fill: new ol.style.Fill({
            color: 'rgba(255, 255, 255, 0.2)'  // Transparente Füllung für Polygone
        }),
        image: new ol.style.Circle({  // Stil für Punkte
            radius: 7,
            fill: new ol.style.Fill({
                color: '#ffcc33'
            })
...
var unsavedGeometries = [];  // Liste der unsaved Geometrien (für neue und geänderte)
var deleteClickActive = false; // Löschmodus deaktiviert
...
function addDrawInteraction(geometryType) {
    removeCurrentInteraction();  // Entferne vorherige Interaktion
    disableDeleteMode();  // Deaktiviere das Löschen, wenn das Zeichnen aktiviert ist

    currentInteraction = new ol.interaction.Draw({
        source: vectorSource,
        type: geometryType  // Setzt den Geometrietyp ('Point', 'LineString', 'Polygon', 'Circle')
    });
    ...
    currentInteraction.on('drawend', function(event) {
        ...
        var coordinates = geometryType === 'Circle'
            ? ol.proj.toLonLat(geometry.getCenter())  // Für Kreise speichern wir das Zentrum
...
function addModifyInteraction() {
    removeCurrentInteraction();  // Vorherige Interaktion entfernen
...
function addDeleteInteraction() {
    removeCurrentInteraction();  // Entferne die vorherige Interaktion

    deleteClickActive = true; // Löschmodus aktivieren
    ...
    map.on('singleclick', function(evt) {
        if (deleteClickActive) {
            ...
            if (feature && confirm('Möchtest du dieses Feature wirklich löschen?')) {
                var geometryId = feature.getId();  // Setze die ID des Features (falls vorhanden)

                unsavedGeometries.push({ id: geometryId, delete: true });
                vectorSource.removeFeature(feature);  // Entferne das Feature von der Karte
...
function sendGeometryToServer(geometry) {
    fetch('http://127.0.0.1:5000/api/geometries', {
        ...
        body: JSON.stringify({ geometry: geometry })  // Sende die Geometrie im JSON-Format
    })
    .then(response => {
        if (!response.ok) {
...
document.querySelector('[data-mode="delete"]').addEventListener('click', function() {
    addDeleteInteraction();  // Löschmodus aktivieren
});
document.querySelector('[data-mode="save"]').addEventListener('click', function() {
    saveChanges();  // Alle unsaved Geometries auf dem Server speichern
});


// Funktion zum Öffnen des Feedback-Formulars
function openFeedbackForm() {
    document.getElementById('feedback-form-container').style.display = 'block'; // Formular anzeigen
}

// Funktion zum Schließen des Feedback-Formulars
function closeFeedbackForm() {
    document.getElementById('feedback-form-container').style.display = 'none'; // Formular ausblenden
}
...
function showThankYouMessage() {
    ...
    thankYouMessage.style.backgroundColor = '#28a745'; // Grün für Erfolg
    ...
    }, 3000); // Nachricht bleibt 3 Sekunden sichtbar
}
...
let selectedOverlay = null; // Um das ausgewählte Overlay zu speichern
...
function makeOverlayDraggable(overlayElement, overlay) {
    ...
    overlayElement.addEventListener('mousedown', function (event) {
        ...
        if (event.target.tagName === 'TEXTAREA' && event.offsetX > event.target.clientWidth - 10 && event.offsetY > event.target.clientHeight - 10) {
            return; // Ignoriere Dragging während des Resizing
        }
        ...
        overlayElement.style.cursor = 'move'; // Ändere den Mauszeiger beim Ziehen
        map.getInteractions().forEach(interaction => {
            if (interaction instanceof ol.interaction.DragPan) {
                interaction.setActive(false); // Deaktiviere das Verschieben der Karte
            }
        });
    });
    ...
    document.addEventListener('mouseup', function () {
        if (isDragging) {
            isDragging = false;
            overlayElement.style.cursor = 'default'; // Setze den Mauszeiger zurück
            map.getInteractions().forEach(interaction => {
                if (interaction instanceof ol.interaction.DragPan) {
                    interaction.setActive(true); // Aktiviere das Verschieben der Karte wieder
                }
            });
        }
    });
...
function adjustFontSize(textElement) {
    const resizeObserver = new ResizeObserver(() => {
        ...
        const newFontSize = boxWidth / 10; // Passe die Schriftgröße proportional zur Boxbreite an
        textElement.style.fontSize = `${newFontSize}px`; // Setze die neue Schriftgröße
    });
...
document.addEventListener('keydown', function (event) {
    if (event.key === 'Delete' && selectedOverlay) {
        map.removeOverlay(selectedOverlay); // Entferne das ausgewählte Overlay von der Karte
        selectedOverlay = null; // Zurücksetzen des ausgewählten Overlays
    }
});
...
map.on('click', function (evt) {
    if (drawTextActive) {
        ...
        // Textbox erstellen
        const container = document.createElement('div'); // Container für Textbox und X
        container.style.position = 'relative';

        const textElement = document.createElement('textarea');
        textElement.placeholder = "Schreibe hier...";
        textElement.style.resize = 'both'; // Erlaubt das Vergrößern/Verkleinern auf allen Seiten
        textElement.style.overflow = 'auto';
        textElement.style.width = '150px'; // Anfangsgröße
        textElement.style.height = '50px'; // Anfangsgröße
        textElement.style.color = 'black';
        textElement.style.backgroundColor = 'white';
        textElement.style.border = '1px solid black';
        textElement.style.padding = '3px';
        textElement.style.fontSize = '15px'; // Startgröße der Schrift
        textElement.style.position = 'relative'; // Notwendig für das Resizing und Verschieben

        // Löschen-Button (nur X)
        const deleteButton = document.createElement('span');
        deleteButton.innerHTML = 'X'; // Einfaches 'X' Symbol
        ...
        deleteButton.addEventListener('click', function () {
            map.removeOverlay(textOverlay); // Entferne das Overlay von der Karte
        });
        ...
        const textOverlay = new ol.Overlay({
            ...
            stopEvent: true, // Ermöglicht die Interaktion mit dem Text
        });
